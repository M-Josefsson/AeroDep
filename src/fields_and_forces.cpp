...
*/
vector3 Get_total_force(Particle& particle, const vector<Particle>& frozen_particles, const InputData& data){
        ...
        for(double x_offset : {-1.0, 0.0, 1.0}){ 
            for(double y_offset : {-1.0, 0.0, 1.0}){ 
                ...
                if(dist < data.interaction_length){  
                    ...
                    F_ext = F_ext + F_vdW_particle_particle(particle, frozen_particle, p_to_p, dist, data.AH132); 
...
* @param p_to_p Vector pointing from the center of an incoming particle to the center of the frozen particle.
* @param r Distance between particle centers (length of p_to_p)  
* @param E0 The strength of the external electric field.
* @param eps Vector containing dielectric constants.
*
...
*
* The jacobian is used in the first oder term in the Taylor expansion of the electric field.
*
* @param frozen_particle An already deposited particle.
* @param p_to_p Vector pointing from the center of an incoming particle to the center of the frozen particle.
* @param r Distance between particle centers (length of p_to_p)  
...
* @param eps Vector containing dielectric constants. 
*
* @return Jacobian of the electric field around the frozen particle at point p_to_p relative to the origin.
*/
Jacobian Gradient_E_field_deposited_particle( const Particle& frozen_particle, const vector3& p_to_p , 
                                                    const double& r, const double& E0, const array<double, 4>& eps){
    double r2 = pow(r,2);    
...
*
* The particles are assumed to be conductive spheres, which gives the force
...
*/
vector3 F_vdW_particle_particle(const Particle& particle, const Particle& frozen_particle, const vector3& p_to_p, 
                                  const double& r, const double& AH){    
    ...
    vector3 F = p_to_p * (-1.0)/r * (32.0*AH/3.0 * pow( R1*R2/(2.0*R1+ S)/(2.0*R2 + S), 2) * (R1*R2 / pow(S, 2)) * 
                                            ( (R1 + R2 + S) / pow(2.0*R1 + 2.0*R2 + S, 2) ) );                
...
* corrections due to the magnetic moment of the already deposited particle. The particles are assumed to be single domained 
* with magnetization susceptilibity Xi. The particles are also assumed to be small enought such that 
* the magnetic field gradient is homogenous through the particle. The force calculation is based on Mikkelsen et al. 
* JoMaMM 293, 578 (2005).
*
* The force is given by
...
*
* The field is at position r around a magnetic dipole with magnetic moment m is 
...
vector3 H_field_dipole(const Particle& frozen_particle, const vector3& p_to_p, const double& dist){
    vector3 H_field = p_to_p * 3.0 * dot(frozen_particle.magnetization, p_to_p)/pow(dist, 2) - frozen_particle.magnetization;    
